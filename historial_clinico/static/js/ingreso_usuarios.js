$(document).ready(function () {
    var table = $("#tabla_usuarios").DataTable({
      columnDefs: [
        ...
        // { targets: 1, visible: false }, // Ocultar la columna de Contraseña
      ],
      "columns": [
        { "data": "id", "visible": false },  // Columna ID oculta 1 
        { "data": "password" }, //  2
        { "data": "last_login" }, //  3
        { "data": "is_superuser" }, // 4
        { "data": "username" }, //  5
        ...
        { "data": "is_active" }, //  10
        { "data": "date_joined" }, //  11
        { "data": "tipo_usuario" }, // 12
        { "data": "especialidad" }, // 13
  ...
  $(document).on("click", ".btn-delete", function () {
    ...
    var nombre = fila.find("td").eq(0).text(); // Apellido Paterno
    var apellido = fila.find("td").eq(1).text(); // Apellido Materno
    var usuario = fila.find("td").eq(3).text(); // Apellido Materno
    

    // Preguntar al usuario si está seguro de eliminar
    Swal.fire({
      title: "¿Estás seguro de eliminar a este usuario?",
      text: "Eliminarás a: " + nombre + " " + apellido + " | Usuario: " + usuario,
      icon: "warning",
              ...
              success: function (response) {
                  if (response.status === "success") {
                      ...
                      table.row.add({
                          ...
                          is_active: response.usuario.is_active ? "Activo" : "Inactivo",
